package celestjava.astro.calc;

import java.util.Vector;
import celestjava.astro.calc.OrthoWayPoint;
...
//  for (double g=start.getG(); g<arrival.getG(); g+=interval)
    ...
    while (go)
    {
      ...
      if (go)
      {
        ...
        double _start = 0.0;
        if (_nsDir == celestjava.astro.calc.Ortho.TO_SOUTH)
        {
          _start = 180.0;
          if (_ewDir == celestjava.astro.calc.Ortho.TO_EAST)
            ari = _start - ari;
          else
            ari = _start + ari;
        }
