...
public class SunPosLoop {
	public static void main(String[] args) {
		...
		adt = new AstroDateTimeLocation(2003, 3, 1, 0, 0, 0, "UTC", longitude, latitude);
		...
		s = String.format("%-12s%-14s%-14s%-14s%-14s%-10s%-10s", "JD", "lambda", "beta", "RA", "dec", "az", "alt");
		System.out.println(s);

		//for (double increment = 0; increment < 1.; increment += (1/24.0)) {
			for (double increment = 0; increment < 1.; increment += .1) {
			adt.advanceJD(increment);
			ac = SPA.getSolarPosition(adt, 1830.14, 67., 820., 11.);
			printTable(adt, ac);
		}
	...
	private static void printTable(AstroDateTimeLocation adt, AstroCoordinate ac) {
		...
		t = String.format(tf, adt.getJD(), ac.eclipticCoord.lambda.getArcDegString(), ac.eclipticCoord.beta.getArcDegString(), ac.equatorialCoord.RA.getHAString(),
				ac.equatorialCoord.dec.getArcDegString(), ac.horizontalCoord.azimuth.getArcDegString(), ac.horizontalCoord.altitude.getArcDegString());
