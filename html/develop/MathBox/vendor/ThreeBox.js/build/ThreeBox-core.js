...
(function(deps) {
	for ( var i in deps) {
		if (!window[i])
			throw "Error: ThreeBox requires " + deps[i];
	}
})({
	'THREE' : 'Three.js',
...
MicroEvent.prototype.on = function() {
...
tQuery.World.registerInstance('addThreeBox', function(element, options) {
	...
	// Make tRenderer.domElement style "display: block" - by default it is
	// inline-block
	...
	if (options.elementResize) {
		ctx.elementResize = ThreeBox.ElementResize.bind(tRenderer, tCamera, element, options).on('resize',
...
tQuery.World.registerInstance('removeThreeBox', function() {
	...
	ctx.cameraControls && this.removeCameraControls() && ctx.cameraControls.stop();
 ...
 * @param {Object}
 *            element The DOM element to size to
 * 
...
ThreeBox.ElementResize = function(renderer, camera, domElement, options) {
	...
	var callback = this.callback = function() {
		var width = Math.floor(domElement.offsetWidth), height = Math.floor(domElement.offsetHeight);
		...
		// Scale
		var ws = Math.floor(width / this.scale), hs = Math.floor(height / this.scale);
...
ThreeBox.OrbitControls.prototype = {
	...
	init : function() {
		this.width = this.element && this.element.offsetWidth, this.height = this.element && this.element.offsetHeight;
	...
	start : function() {
		...
		this._mouseDown = function(event) {
			that.width = that.element && that.element.offsetWidth, that.height = that.element
					&& that.element.offsetHeight;

			that.drag = true;
			that.lastHover = that.origin = {
				x : event.pageX,
				y : event.pageY
			};
		...
		this._mouseMove = function(event) {
			if (that.drag) {
				var relative = {
					x : event.pageX - that.origin.x,
					y : event.pageY - that.origin.y
				}, delta = {
					x : event.pageX - that.lastHover.x,
					y : event.pageY - that.lastHover.y
				};
				that.lastHover = {
					x : event.pageX,
					y : event.pageY
 ...
 * 
 * @param {Object}
 *            element DOM element to track mouse movement on.
 * @param {Object}
 *            options Options for ThreeBox.
 */
...
ThreeBox.preload.html = function(file, name, callback) {
	new microAjax(file, function(res) {
		...
		while (match = res
				.match(/^(<script\s*>|<script[^>]*type=['"]text\/javascript['"][^>]*>)([\s\S]+?)<\/script>$/m)) {
			try {
				/*
				 * var script = document.createElement('script'); script.type =
				 * 'text/javascript'; script.innerHTML = match[2];
